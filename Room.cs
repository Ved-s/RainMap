	...
	public class Room
	{
		...
		public void ApplyLevelToShader(Effect effect, int screenIndex, Renderer renderer)
		{
			if (Main.UseParallax)
			{
				...
				effect.Parameters["ParallaxDistance"]?.SetValue(0 /*10 / renderer.Size.X * renderer.Scale*/);
		...
		void UpdateWater()
		{
			...
			for (int num8 = 0; num8 < Water.Surface.GetLength(0); num8++)
			{
				...
				point.nextHeight += MathHelper.Lerp(-waveAmp, waveAmp, (float)0.5f) * 0.005f;
				point.nextHeight *= 0.99f;
				//if (this.room.roomSettings.DangerType != RoomRain.DangerType.None)
				//{
				//    point.nextHeight += (float)Math.Lerp(-1f, 1f, 0.5f) * this.room.world.rainCycle.ScreenShake * this.room.roomSettings.RumbleIntensity;
				//}
				...
				for (int num9 = 0; num9 < 2; num9++)
				{
					...
					//if (this.room.roomSettings.DangerType != RoomRain.DangerType.None)
					//{
					//    Water.Surface[num8, num9].pos += Custom.DegToVec(0.5f * 360f) * this.room.world.rainCycle.MicroScreenShake * 4f * this.room.roomSettings.RumbleIntensity;
					//}
		...
		void DrawWater(Renderer renderer, int screenIndex)
		{
			...
			float parallaxDistance = 0; //10 / renderer.Size.X * renderer.Scale;
			...
			float pixLoss = 1;// 1 / renderer.Scale;
			if (pixLoss > 0)
